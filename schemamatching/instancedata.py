...
class SchemaMatcher:
    ...
    def get_classifier_results(self, xml1=None, xml2=None):
        if xml1 is None: xml1 = self.xml1
        if xml2 is None: xml2 = self.xml2
        ...
        self.classifier_results = pd.DataFrame(np.zeros(results_shape),
                                               index=xml1_data.keys(),
                                               columns=xml2_data.keys())
        ...
        for tag in xml1_data:
            ...
            for p in predictions:
                self.classifier_results.loc[tag, p] += 1.0 / total

        return self.classifier_results

    def get_word2vec_results(self):
        results = compare_tag_names(self.xml1, self.xml2)
        self.word_embedding_results = results
        return results

    def do_similarity_flooding(self):
        if self.word_embedding_results is None:
    ...
    #                  mean_difference]
    #     return {f.__name__: f(self.true_mappings_matrix, self.classifier_results)
    #             for f in functions}

    def get_score(self, scoring_function):
        return scoring_function(self.true_mappings_matrix, self.classifier_results)

    # def _do_internal_comparison(self):
    ...
    #     true, predictions = self._do_internal_comparison()
    #     return {
...
def score_pipeline(xml, pipeline, scoring_methods):
    ...
    x = features['content']
    y = features['tag']
    results = {scoring_method: _average(cross_val_score(
        pipeline, x, y, scoring=scoring_method)) for scoring_method in
        scoring_methods}
    return results
...
def compare_xmls(xml1, xml2, model=None,
    ...
    if model is None: model = DecisionTreeClassifier(random_state=42)
...
def hungarian_algorithm(results):
    ...
    return [(results.index[row], results.index[col]) for
            row, col in zip(row_index, col_index)]
